...
let select = (achv_id,user_id) => {
    return db
    .query("SELECT * FROM AchievementAccount where achv_id = $1 AND user_id = $2;", [achv_id,user_id])
    .then(function({rows}) {
...
let remove = (achv_id,user_id) => {
  return db
  .query("DELETE FROM AchievementAccount where achv_id = $1 AND user_id = $2; ", [achv_id,user_id])
  .then(function({rows}) {
...
function create(patch) {
  ...
  return db
    .query(
      `INSERT INTO achievementaccount(achv_id,user_id) VALUES (${modifier});`)
    .then(function({ rows }) {
      return {"status" : "Success"};
    })
    .catch((err) => {throw (err)});
...
function update(patch) {
  ...
  return db
    .query(
      `UPDATE AchievementAccount SET ${modifier} WHERE (user_id = ${user_id} AND achv_id = '${sec_id}') RETURNING *;`, 
      Object.values(patch)
    )
    .then(function({ rows }) {
      if (rows.length !== 1) {
