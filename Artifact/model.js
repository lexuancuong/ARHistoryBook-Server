...
let select = (sec_id) => {
    return db
    .query("SELECT * FROM artifact where id = $1", [sec_id])
    .then(function({rows}) {
...
let remove = (sec_id) => {
  return db
  .query("DELETE FROM artifact where id = $1", [sec_id])
  .then(function({rows}) {
...
function create(patch) {
  ...
  return db
    .query(`INSERT INTO artifact(id,name,description) VALUES (${modifier});`)
    .then(function({ rows }) {
      return {"status" : "Success"};
    })
    .catch((err) => {throw (err)});
...
function update(patch) {
  ...
  return db
    .query(
      `UPDATE artifact SET ${modifier} WHERE id = '${id}' RETURNING *;`,
      Object.values(patch)
    )
    .then(function({ rows }) {
      if (rows.length !== 1) {
